                    ...
                    do {
                        ...
                        switch (choice) {
                            ...
                            case "Cancel Tickets":{
                                ...
                                ps = conn.prepareStatement("select bus_id from passengers where booking_id = ?");
                                ps.setInt(1,booking_id);
                                ResultSet rs = ps.executeQuery();
                                rs.next();
                                int bus_id = rs.getInt(1);
                            ...
                            case "View Bookings":
                            {
                                ps = conn.prepareStatement("select * from passengers where booking_id in(select booking_id from booking where user_id = ?) order by bus_id,seat_no;");
                                ps.setInt(1,user_id);
                                ResultSet rs =ps.executeQuery();
                                while (rs.next()) {
